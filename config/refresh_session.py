...
# Force UTF-8 output immediately, before any print calls
if sys.stdout.encoding != 'utf-8':
    sys.stdout.reconfigure(encoding='utf-8', errors='replace')
if sys.stderr.encoding != 'utf-8':
    sys.stderr.reconfigure(encoding='utf-8', errors='replace')

# Try importing cryptography or warn the user
try:
    from cryptography.hazmat.primitives.ciphers.aead import AESGCM
except ImportError:
    result = {"success": False, "error": "Missing 'cryptography' module. Run: .venv\\Scripts\\pip.exe install cryptography"}
...
def extract_session():
    try:
        ...
        try:
            ...
            return {"success": False, "error": "CapCut Desktop đang mở và khóa dữ liệu. Vui lòng tắt phần mềm CapCut Desktop và thử lại."}
        ...
        if not cookie_str:
            return {"success": False, "error": "CapCut Desktop chưa được đăng nhập. Vui lòng đăng nhập trên CapCut Desktop trước."}
            
        # Update config/capcut_headers.json
        ...
        if os.path.exists(headers_path):
            ...
            return {"success": True, "message": "Đã kết nối thành công với CapCut Desktop. Đã cập nhật phiên làm việc mới!"}
        else:
            return {"success": False, "error": "Không tìm thấy file capcut_headers.json để cập nhật."}
