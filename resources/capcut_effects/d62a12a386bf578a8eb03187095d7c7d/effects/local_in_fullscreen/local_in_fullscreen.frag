            ...
            float _contrast, float _blend, vec2 _screen_size, out vec4 o_res)
{
    ...
    vec4 neg_col = vec4(1.4-offset_col.rgb, offset_col.a);
    neg_col.rgb = clamp(neg_col.rgb, vec3(0), vec3(1));
...
vec4 mix1(vec4 r1, vec4 r2, float _a) {
    return r1*(1.-_a) + r2;
}
...
void cylinder_mapping(vec2 _u, vec2 _corner_pos, vec2 _fold_pos, float _fold_radius, vec2 _extra_size, inout vec4 res)
{
    ...
    upside_col.rgb += hash21(upside_uv*100.) * 0.03;
    upside_col *= clamp(mirror_upside_mask+mirror_mask, 0., 1.);

    // res = upside_col*(1.-res.a) + res;
    // res = bottom_col*(1.-res.a) + res;
